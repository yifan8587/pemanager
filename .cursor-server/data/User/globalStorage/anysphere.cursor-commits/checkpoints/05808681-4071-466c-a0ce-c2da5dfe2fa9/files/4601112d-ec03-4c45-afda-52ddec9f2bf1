...
import { Share, Refresh, Download, Upload, View, Plus } from '@element-plus/icons-vue'
...
async function importSystemRoute(row) {
  ...
  try {
    await ElMessageBox.confirm(
      `将内核路由 ${payload.dst} via ${payload.gateway || '(on-link)'} dev ${payload.dev} 导入为「路由表」？`,
      '导入确认',
      { type: 'info' },
    )
  } catch {
...
async function importAllSystemRoutes() {
  const items = (systemSnap.value.routes || [])
    .map(routeRowToImportPayload)
    .filter(Boolean)
  if (!items.length) {
    ElMessage.warning('没有可导入的系统路由')
    return
  }
  try {
    await ElMessageBox.confirm(`将批量导入 ${items.length} 条系统路由为意图？`, '批量导入', {
      type: 'warning',
    })
  } catch {
    return
  ...
  try {
    const data = await routeApi.importRoutesFromSystem(items)
    ElMessage.success(`成功导入 ${data.created_count || 0} 条；失败 ${data.errors?.length || 0} 条`)
    activeTab.value = 'intent'
    await load()
  } catch (e) {
    ElMessage.error(e?.response?.data?.detail || e.message || '批量导入失败')
  }
}
...
// ===== 下发：WG（ip route + 持久化脚本）=====
const wgPreview = ref({ commands: [], script: '', script_path: '' })
const loadingWgPreview = ref(false)
const wgApplying = ref(false)
const wgWriteScript = ref(true)
...
async function fetchWgPreview() {
  ...
  try {
    ...
    wgPreview.value = data || {}
    ElMessage.success(`已生成 WireGuard 路由命令（共 ${data?.count || 0} 条）`)
  } catch (e) {
...
async function applyWg() {
  try {
    await ElMessageBox.confirm(
      `将通过 ip route replace 下发 WireGuard 路由，并${wgWriteScript.value ? '同时写入持久化脚本' : '不写入持久化脚本'}。继续？`,
      'WireGuard 路由下发',
      { type: 'warning' },
    )
  } catch {
  ...
  try {
    const { data } = await routeApi.applyWireguardRoutes({ write_script: wgWriteScript.value })
    lastWgResult.value = data
    if (data.ok) {
      ElMessage.success(`已下发 ${data.applied} 条 WireGuard 路由${data.script_written ? '（含持久化脚本）' : ''}`)
    } else {
      await ElMessageBox.alert(
        `<pre style="white-space:pre-wrap;font-size:12px;max-height:420px;overflow:auto">${JSON.stringify(data, null, 2)}</pre>`,
        '下发失败',
...
<template>
  <div class="page">
    <PageHeader title="静态路由" description="维护静态路由表：非 WG 接口经 netplan 持久化；WireGuard 接口经 ip route 单独下发" :icon="Share">
      <template #actions>
    ...
    <div class="kpi-row">
      ...
      <div class="kpi">
        ...
        <div class="kpi-hint">经 netplan 持久化</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">WireGuard 路由</div>
        <div class="kpi-value">{{ stats.wg }}</div>
        <div class="kpi-hint">经 ip route 单独下发</div>
      </div>
      <div class="kpi">
    ...
    <el-tabs v-model="activeTab" type="border-card" class="route-tabs">
      ...
      <el-tab-pane name="intent">
        ...
        <el-table :data="filteredRows" border size="small" v-loading="loading" stripe>
          ...
          <el-table-column label="客户 / 资源 IP" min-width="180" show-overflow-tooltip>
            <template #default="{ row }">
              <div v-if="row.customer_name || row.ip_address">
                <div v-if="row.customer_name" class="cust">{{ row.customer_name }}</div>
      ...
      <el-tab-pane name="system">
        <template #label>
          <span><el-icon><View /></el-icon> 系统路由表（ip route）</span>
        </template>
        ...
        <div class="toolbar">
          ...
          <el-button size="small" type="warning" :icon="Download" @click="importAllSystemRoutes">批量导入</el-button>
        </div>
        ...
        >
          ...
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" :disabled="!row.dev" :icon="Download" @click="importSystemRoute(row)">
                导入意图
              </el-button>
            </template>
          </el-table-column>
        ...
        <el-row :gutter="16">
          ...
          <el-col :xs="24" :md="10">
            <el-card shadow="never" class="dep-card wg-card">
              <template #header>
                <div class="card-hd">
                  <span class="title">B · 下发 WireGuard 路由（ip route）</span>
                  <el-tag type="warning" size="small">{{ stats.wg }} 条</el-tag>
                </div>
              </template>

              <p class="hint">
                WireGuard 接口的私钥由 <code>interfacemanage</code> 单独管理；本模块仅以
                <code>ip route replace</code> 下发与之关联的路由，并可选写入
                <code>networkd-dispatcher</code> 持久化脚本以在 WG 接口 up 时自动重放。
              </p>

              <div class="deploy-actions">
                <el-button :icon="View" :loading="loadingWgPreview" @click="fetchWgPreview">1. 预览命令</el-button>
                <el-checkbox v-model="wgWriteScript" border>同时写入持久化脚本</el-checkbox>
                <el-button type="primary" :loading="wgApplying" @click="applyWg">2. 下发 (ip route)</el-button>
              </div>

              <el-form-item label="ip route 命令">
                <el-input
                  :model-value="(wgPreview.commands || []).join('\n')"
                  type="textarea"
                  :rows="6"
                  readonly
                  placeholder="点击「预览命令」生成将要执行的 ip route 命令"
                  class="mono"
                />
              </el-form-item>

              <el-form-item :label="`脚本 (${wgPreview.script_path || '/etc/networkd-dispatcher/routable.d/91-pemanager-wg-routes.sh'})`">
                <el-input
                  :model-value="wgPreview.script || ''"
                  type="textarea"
                  :rows="6"
                  readonly
                  placeholder="点击「预览命令」生成持久化脚本"
                  class="mono"
                />
              </el-form-item>
